...
import {
  CreatureInputForm,
  CreatureOutputCard,
  type CreatureInputData,
  type GeneratedCreature,
} from '@/components/forge/creature';
...
export default function CreatureForgePage(): JSX.Element {
  ...
  // The forge hook
  const forge = useForge<CreatureInputData, GeneratedCreature>({
    campaignId,
    forgeType: 'creature',
    ...
    getTextContent: (output) => {
      ...
      return extractTextForScanning({
        vivid_description: output.soul?.vivid_description,
        behavior: output.soul?.behavior,
        habitat: output.soul?.habitat,
        ecology: output.soul?.ecology,
        tactics: output.brain?.tactics,
        motivations: output.brain?.motivations,
        secret: output.brain?.secret,
        lair_description: output.brain?.lair_description,
        treasure_description: output.treasure?.treasure_description,
      });
    },
  ...
  // Helper function to infer relationship type for creatures
  const inferCreatureRelationshipType = (
    theirType: string,
    theirSubType?: string
  ): string => {
    if (theirType === 'location') return 'inhabits';
    if (theirType === 'npc') return 'connected_to';
    if (theirType === 'creature') return 'related_to';
    if (theirType === 'faction') return 'allied_with';
    return 'connected_to';
  };
  ...
  const handleCommit = async (): Promise<void> => {
    ...
    if (stubId) {
      try {
        ...
        const creatureSubType = forge.output.sub_type || 'creature';
        const { error } = await supabase
          .from('entities')
          .update({
            name: forge.output.name,
            sub_type: creatureSubType,
            ...
            summary: forge.output.soul?.vivid_description?.substring(0, 200),
            description: forge.output.soul?.vivid_description,
            forge_status: 'complete',
            attributes: {
              treasure: forge.output.treasure,
              srd_base: forge.output.srd_base,
              is_stub: false,
              needs_review: false,
              history: [
                ...(existingHistory as Array<Record<string, unknown>>),
                {
                  ...
                  note: 'Completed via Creature forge',
                },
              ],
            },
          })
        ...
        if (generationReferencedEntities.length > 0) {
          const relationshipPromises = generationReferencedEntities.map((refEntity) =>
            supabase.from('relationships').insert({
              ...
              relationship_type: inferCreatureRelationshipType(
        ...
        if (stubsToCreate.length > 0) {
          ...
          console.log('[Creature Forge] Created stubs during flesh-out:', createdStubs)
        }

        toast.success('Creature fleshed out and saved!');
        router.refresh();
      ...
      if (result.success && result.entity) {
        ...
        toast.success('Creature saved to Memory!');
  ...
  const handleGenerate = async (input: CreatureInputData): Promise<void> => {
    ...
    try {
      ...
      if (result.success) {
        toast.success('Creature generated successfully!');
      }
    } catch (error) {
      toast.error(
  ...
  const forgeTitle = stubContext ? `Flesh Out: ${stubName}` : 'Creature Forge';

  const forgeDescription = stubContext
    ? 'Complete this stub entity with full creature details'
    : 'Generate unique monsters and beasts with full stat blocks';
    ...
    <ForgeShell
      ...
      outputSection={
        forge.output ? (
          ...
          <EmptyForgeState
            forgeType="Creature"
            description='Describe your creature concept and click "Generate Creature" to forge it.'
          />
        )
      }
      commitPanel={
