...
export function ItemInputForm({
  ...
  const randomizeType = (): void => {
    const randomType = RANDOMIZABLE_TYPES[Math.floor(Math.random() * RANDOMIZABLE_TYPES.length)]
    setItemType(randomType.value)
  }

  const randomizeRarity = (): void => {
    const randomRarity = RANDOMIZABLE_RARITIES[Math.floor(Math.random() * RANDOMIZABLE_RARITIES.length)]
    setRarity(randomRarity.value)
  }

  const randomizeDmSlug = (): void => {
    const randomSeed = DM_SLUG_SEEDS[Math.floor(Math.random() * DM_SLUG_SEEDS.length)]
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <Select value={itemType} onValueChange={setItemType} disabled={isGenerating}>
        ...
        <div className="space-y-2">
          ...
          <Select value={rarity} onValueChange={setRarity} disabled={isGenerating}>
        ...
        <div className="space-y-2">
          ...
          <Select value={magicalAura} onValueChange={setMagicalAura} disabled={isGenerating}>
      ...
      <div className="flex items-center justify-between p-3 border rounded-lg bg-muted/30">
        <div>
          <Label htmlFor="is-identified" className="cursor-pointer">Is Identified?</Label>
          <p className="text-xs text-muted-foreground">
      ...
      <Button
        type="submit"
        disabled={!dmSlug.trim() || isGenerating || remainingGenerations <= 0}
        className="w-full"
        size="lg"
      >
        {isGenerating ? (
